    ...
    enforcement_profiles: [
      ...
      },
      {
        ...
        does_not_hold: 'the ledger is not equivalent to a database primary key: '
          ...
          + 'a pre-receive hook that refuses ledger deletes and overwrites IS now '
          + 'shipped (installLedgerHook), and it closes the push-level vector ONLY '
          + 'WHEN INSTALLED on the bare repository that serves pushes — an '
          + 'uninstalled hook protects nothing, and a writer with filesystem '
          + 'access bypasses receive-pack entirely, so trusted storage stays an '
          + 'assumption rather than something the hook enforces. Git has no '
